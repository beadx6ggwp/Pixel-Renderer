...
這份 audit 是對 `learning/cpp/` 的第二輪課程設計審查。它不是新的教學章節，而是接下來重寫、加深、補章節、整理 index / README 的依據。
...
```text
learning/cpp/impl/i01_current_renderer_architecture_walkthrough.html
```
...
```text
learning/cpp/impl/i16_verification_golden_image_regression_testing.html
```
...
```text
learning/cpp/impl/i17_header_boundary_dependency_hygiene.html
```
...
```text
learning/cpp/theory/t15_numeric_semantics_pixel_formats.html
```
...
```text
learning/cpp/impl/i18_diagnostics_sanitizers_static_analysis.html
```
...
```text
learning/cpp/impl/i19_preprocessor_compile_time_configuration.html
```
...
```text
learning/cpp/impl/i20_dependency_management_third_party_integration.html
```
...
```text
learning/cpp/theory/t16_api_design_conventions.html
```
...
```text
learning/cpp/theory/t17_smart_pointer_ownership_adapters.html
```
...
```text
learning/cpp/theory/t18_casts_type_punning_bit_cast.html
```
...
```text
learning/cpp/theory/t22_const_cv_value_category.html
```
...
```text
learning/cpp/theory/t23_overload_conversion_operator_design.html
```
...
```text
learning/cpp/theory/t24_type_deduction_auto_decltype_forwarding.html
```
...
```text
learning/cpp/theory/t25_static_storage_global_state_initialization.html
```
...
```text
learning/cpp/theory/t26_regular_types_equality_hashing_ordering.html
```
