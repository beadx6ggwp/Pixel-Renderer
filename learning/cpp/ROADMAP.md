...
這份 roadmap 的目的不是再加一份待辦清單，而是把目前 `learning/cpp/` 重新整理成一套連貫課程。
...
第二輪審查詳見 `learning/cpp/AUDIT.md`。Phase A/B/C/D/D.1 已完成：結構缺口已補、骨架章已加深、早期章節已補 SVG / memory-style 圖解、index 已改成統一版入口、舊 HTML 章節已移除。
